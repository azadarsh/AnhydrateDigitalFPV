...
// Relay flag should be ignored, it's transparent, Anhydrate handles the relaying if needed
...
// Same flags denote the ones that are actually alocated by Anhydrate to the plugin;
...
// Anhydrate will call this methods to get the segment from the plugin so that it can be (re)send over air.
...
// The plugin should return the type of video streams it generates/handles so that Anhydrate can disable it's own handling of such streams. (For example, you can't have both Anhydrate and your plugin accessing and handling the CSI video port on the Pi.)
// This method is mandatory to be implemented if your plugin requested video capabilities.
// Failing to return the right type of video stream the plugin handles (and the video hardware it accesses) can result in undefined behavior in Anhydrate.
...
// If the plugin returns 0, Anhydrate will mark that the video stream from the plugin is not available.
...
// It's up to the plugin to store the captured video stream and provide it to Anhydrate, in a segmented way, when requested by Anhydrate with the call: core_plugin_has_pending_tx_data
