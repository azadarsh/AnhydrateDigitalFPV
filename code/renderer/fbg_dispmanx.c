...
//#endif
    ...
    dispmanx_context->alfa_resource = vc_dispmanx_resource_create(dispmanx_context->resource_type, info.width, info.height, &vc_image_ptr);
    if (dispmanx_context->front_resource == 0) {
        fprintf(stderr, "fbg_dispmanxSetup: vc_dispmanx_resource_create failed for display %i\n", displayNumber);
        vc_dispmanx_resource_delete(dispmanx_context->back_resource);
        vc_dispmanx_display_close(dispmanx_context->display);
...
#else
    vc_dispmanx_resource_write_data(dispmanx_context->back_resource, dispmanx_context->resource_type, dispmanx_context->pitch, fbg->back_buffer, dispmanx_context->dst_rect);
#endif
}
