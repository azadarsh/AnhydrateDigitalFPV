...
int portNumber = 1;
data_packet_Anhydrate_telemetry* pdpvt;

void process_data(u8* buffer, int length)
{
   data_packet_header dph;
   memcpy(&dph, buffer, sizeof(dph) );
   memcpy(pdpvt, buffer+sizeof(dph), sizeof(data_packet_Anhydrate_telemetry) );
   if ( (dph.packetType == PACKET_TYPE_Anhydrate_TELEMETRY) && radio_validate_crc((data_packet_header*)buffer) )
   {
...
int main(int argc, char *argv[])
{
   ...
   pdpvt = shared_mem_Anhydrate_telemetry_open(false);
   reset_data_packet_Anhydrate_telemetry(pdpvt);
      ...
      for(int i=0; i<hardware_get_radio_interfaces_count(); i++)
      {
         ...
         if(FD_ISSET(pNICInfo->monitor_interface.selectable_fd, &readset))
         {
            ...
            if ( NULL != buffer )
            {
               ...
               if ( length == (sizeof(data_packet_header) + sizeof(data_packet_Anhydrate_telemetry)) )
               {
                  printf("\nData is: Anhydrate telemetry.\n");
                  process_data(buffer, length);
               }
               if ( length == (sizeof(data_packet_header) + sizeof(data_packet_Anhydrate_telemetry)+sizeof(data_packet_fc_telemetry)) )
               {
                  printf("\nData is: Anhydrate telemetry all.\n");
